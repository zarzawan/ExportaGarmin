...
class XlsxContractTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        try:
            ...
            raise unittest.SkipTest("openpyxl no está instalado")

    def test_workbook_has_expected_tables_preserves_null_and_escapes_formulas(self):
        from openpyxl import load_workbook

        raw_identifier = "987654321"
        ...
        model = {
            ...
            "race_context": {},
            "period_summary": {"running": {"distance_m": 0}},
            "race_analysis": {},
            "weekly_timeline": [],
            "daily_health": [{
                "date": "2026-01-01",
            ...
            "activities": [{
                ...
                "activity_series": {
                    "metric_descriptors": [
                        ...
                        {
                            "field": "elevation_raw",
                            "source_field": "directElevation",
                            "source_unit": "meter",
                        },
                        {
                            "field": "elevation_raw",
                            "source_field": "directEnhancedElevation",
                            "source_unit": "meter",
                        },
                    ],
                    "samples": [
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(expected_sheets, set(workbook.sheetnames))

            readme = workbook["LEEME"]
            readme_headers = {
                cell.value: cell.column for cell in readme[1]
            }
            self.assertIn(
                "exportación parcial",
                readme.cell(
                    2,
                    readme_headers["instruccion"],
                ).value,
            )

            days = workbook["DIAS"]
            headers = {
                cell.value: cell.column for cell in days[1]
            }
            self.assertEqual(0, days.cell(2, headers["steps"]).value)
            self.assertIsNone(
                days.cell(2, headers["sleep_score"]).value
            )

            activity_gear = workbook["ACTIVIDAD_EQUIPAMIENTO"]
            activity_gear_headers = {
                cell.value: cell.column for cell in activity_gear[1]
            }
            self.assertEqual(
                activity_ref,
                activity_gear.cell(
                    2,
                    activity_gear_headers["activity_ref"],
                ).value,
            )
            self.assertEqual(
                "Nike",
            ...
            self.assertEqual(
                '\'=HYPERLINK("https://invalid.example/gear")',
                gear_formula_like.value,
            )
            self.assertNotEqual("f", gear_formula_like.data_type)

            series_descriptors = workbook["SERIES_DESCRIPTORES"]
            descriptor_headers = {
                cell.value: cell.column for cell in series_descriptors[1]
            }
            descriptor_columns = [
                series_descriptors.cell(
                    row,
                    descriptor_headers["column_name"],
                ).value
                for row in range(2, 5)
            ]
            self.assertEqual(
                [
                    "elapsed_duration_raw",
                    "elevation_raw",
                    "elevation_raw_2",
                ],
                descriptor_columns,
            ...
            self.assertEqual(
                ...
                series_descriptors.cell(
                    3,
                    descriptor_headers["source_unit"],
                ).value,
            )

            series = workbook["SERIES_ACTIVIDAD"]
            series_headers = {
                cell.value: cell.column for cell in series[1]
            }
            self.assertEqual(
                activity_ref,
                series.cell(2, series_headers["activity_ref"]).value,
            )
            self.assertEqual(
                0,
                series.cell(2, series_headers["sample_index"]).value,
            )
            self.assertIsNone(
                series.cell(2, series_headers["elevation_raw"]).value,
            )
            self.assertEqual(
                642.5,
                series.cell(2, series_headers["elevation_raw_2"]).value,
            )

            journal = workbook["DIARIO"]
            journal_headers = {
                cell.value: cell.column for cell in journal[1]
            }
            formula_like = journal.cell(
                2,
                journal_headers["note"],
            )
            self.assertEqual(
                "'=HYPERLINK(\"https://invalid.example\")",
                formula_like.value,
            )
            self.assertNotEqual("f", formula_like.data_type)
            ...
            self.assertNotIn(raw_identifier, xml_text)

    def test_large_series_are_omitted_from_xlsx_and_reported(self):
        from openpyxl import load_workbook
        ...
        model = {
            ...
            "activities": [{
                ...
                "activity_series": {
                    ...
                    "samples": (
                        [sample]
                        * (XLSX_MAX_ACTIVITY_SERIES_SAMPLES + 1)
                    ),
                },
            }],
            "data_quality": {},
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            workbook = load_workbook(output, data_only=False)
            try:
                self.assertEqual(1, workbook["SERIES_ACTIVIDAD"].max_row)
                quality_values = {
                    row[0]: row[1]
                    for row in workbook["CALIDAD_DATOS"].iter_rows(
                        min_row=2,
                        values_only=True,
                    )
                }
                self.assertEqual(
                    "omitted_size_limit",
                    quality_values["xlsx.activity_series.status"],
                )
                self.assertEqual(
                    XLSX_MAX_ACTIVITY_SERIES_SAMPLES + 1,
                    quality_values[
                        "xlsx.activity_series.available_samples"
                    ],
                )
                self.assertEqual(
                    0,
                    quality_values["xlsx.activity_series.exported_samples"],
                )
                self.assertIn(
                    "El formato TXT conserva todas las muestras",
                    quality_values["xlsx.activity_series.note"],
                )
            finally:
                workbook.close()
            self.assertEqual(
                XLSX_MAX_ACTIVITY_SERIES_SAMPLES + 1,
                len(
                    model["activities"][0]["activity_series"]["samples"]
                ),
            )
